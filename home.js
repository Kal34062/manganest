...
document.addEventListener("DOMContentLoaded", () => {
  ...
  onAuthStateChanged(auth, (user) => {
    const userInfo = document.getElementById("user-info");
    if (user && userInfo) {
      userInfo.textContent = `Logged in as: ${user.displayName || "User"} (${user.email})`;
    } else if (userInfo) {
      userInfo.textContent = "Browsing as Guest";
    }
  });
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener("click", () => {
      signOut(auth).then(() => window.location.href = "Singup.html");
    });
  }
  ...
  function getCoverUrl(manga, size = 256) {
    ...
    return `https://via.placeholder.com/${size}x${size * 1.4}?text=No+Cover`;
  }
  ...
  function updateHero() {
    ...
    heroSynopsis.textContent = manga.attributes?.description?.en
      ? manga.attributes.description.en.replace(/\[.*?\]\(.*?\)/g, "").substring(0, 200) + "..."
      : "No synopsis available";
    hero.style.backgroundImage = `url(${getCoverUrl(manga, 512)})`;
